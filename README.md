...
* `kargo-htb.js` - This is your partner module file, by default it contains a template divided into multiple sections which need to be completed.
* `kargo-htb-validator.js` - This is the validator file for the configuration object that will be passed into your module.
* `kargo-htb-exports.js` - A file that contains all of the modules exports (i.e. any functions that need to be exposed to the outside world).
...
1. <b>Complete the kargo-htb.js file </b>
    * kargo-htb.js is where all of your adapter code will live.
...
2. <b>Complete the kargo-htb-validator.js file</b>
...
3. <b>Complete the kargo-htb-exports.js file</b>
    * This file will contain any functions that need to be exported or exposed to the outside world. Things like render functions, custom callbacks, etc. Any legacy render functions will also need to be exposed here. Anything added to the `shellInterface.KargoHtb` will be accessible through `window.headertag.KargoHtb`
...
for (var i = 0; i < 10; i++) {
    ...
    if (w.headertag) {
        try {
            w.headertag.KargoHtb.render(document, %%PATTERN:TARGETINGMAP%%, '%%WIDTH%%', '%%HEIGHT%%');
            break;
        } catch (e) {
            continue;
        }
...
In this section you will be filling out the kargo-htb.js, kargo-htb-exports.js, and the kargo-htb-validator.js files to create your module.

### Step 0: Config Validation (`kargo-htb-validator.js`)
...
Once you have a basic idea of what this will look like, and how you will uniquely identify each slot on your server (via xSlot placementId or other inventory codes) you will need to validate this configuration. This validation will be performed by the wrapper using the `kargo-htb-validator.js` file.

The `kargo-htb-validator.js` file contains a single export, a `partnerValidator` function, that takes in the configuration object that will be fed to your module's constructor (refer to [Configuration](#configuration) for an example layout) and validates it via type checks. The type checks are performed using an external library called `schema-inspector`, for which the documentation can be found here https://github.com/Atinux/schema-inspector.

We have provided a very basic validation schema in `kargo-htb-validator.js` that is based off of the example `mockPartnerConfig.js` object found in the `spec/support` directory for testing (refer to the [Testing](#testing) section for the testing structure).

Once you have filled this file out, you can continue actually writing your module!

### Step 1: Partner Configuration (`kargo-htb.js`)
...
### Step 2: Generate Request URL (`kargo-htb.js`)
...
### Step 3: Response Callback (`kargo-htb.js`)
...
### Step 4: Parsing and Storing Demand (`kargo-htb.js`)
...
### Step 5: Rendering Pixel (`kargo-htb.js`)
...
### Step 6: Exports (`kargo-htb-exports.js`)
...
```javascript
shellInterface.KargoHtb = { //shell interface is the window variable that is accessable through the window object, currently this will always be window.headertag
...
```javascript
if (__directInterface.Layers.PartnersLayer.Partners.KargoHtb) {
    shellInterface.KargoHtb = shellInterface.KargoHtb || {};
    shellInterface.KargoHtb.adResponseCallback = __directInterface.Layers.PartnersLayer.Partners.KargoHtb.adResponseCallback;
}
```
...
if (__directInterface.Layers.PartnersLayer.Partners.KargoHtb) {
    shellInterface.KargoHtb = shellInterface.KargoHtb || {};
    shellInterface.KargoHtb.adResponseCallbacks = __directInterface.Layers.PartnersLayer.Partners.KargoHtb.adResponseCallbacks;
}
```
